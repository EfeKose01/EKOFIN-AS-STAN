...
# NOT: st.set_page_config artık burada değil — tek giriş noktası streamlit_app.py
# olduğu için sayfa konfigürasyonu orada bir kez çağrılıyor. Bu dosya artık
# streamlit_app.py içinden bir "sayfa fonksiyonu" (run_streamlit_app) olarak kullanılıyor.
# CUSTOM_CSS de ortak kullanım için pages_ui/theme.py içine taşındı.
from pages_ui.theme import CUSTOM_CSS  # noqa: E402
...
def _get_logged_in_portfolio_context() -> str:
    """Sohbet ekranı herkese açık olsa da, kullanıcı AYNI ZAMANDA myPortfolio'ya
    giriş yapmışsa (Streamlit oturumu sayfalar arasında paylaşılır) onun portföyünü
    RAG asistanına bağlam olarak ekler. Her adımda sessizce başarısız olur —
    bu tamamen "varsa iyi olur" nitelikte bir kişiselleştirme, sohbeti asla kırmamalı.
    """
    try:
        from backend.database import get_db
        from backend.services.news_service import build_portfolio_context_block
        from backend.services.portfolio_service import SECTOR_MAP, list_portfolio
        ...
        with get_db() as db:
            ...
            if not items:
                return ""
            symbols_with_sectors = [(it.symbol, SECTOR_MAP.get(it.symbol, "Diğer")) for it in items]
            return build_portfolio_context_block(symbols_with_sectors)
    except Exception:
        return ""


def run_tool_calling_logic(chat_history: List[Dict[str, Any]], persona: str) -> str:
    system_prompt = PERSONA_PROMPTS.get(persona, PERSONA_PROMPTS["Genel Asistan"]) + _get_logged_in_portfolio_context()
